...
import lessons13streamapi.model.UserDB;
import lessons13streamapi.model.UserDTO;
...
//                .collect( Collectors.toList());
    ...
    public static List<UserDB> findUsersByYear ( final List<UserDB> users, final int year ) {
        ...
        for (UserDB userDB : users) {
            if ( userDB.getBirthdayYear ( ) == year ) {
                list.add ( userDB );
            }
        }
        return list;
...
//            map.computeIfAbsent ( user.getBirthdayYear ( ), k -> new TreeSet<> ( ) ).add ( lastName );
//        }
//        return map;
